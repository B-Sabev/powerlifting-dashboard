...
- 2026-08-01: Demo check-in sleep generation reworked — working days (Mon–Fri) are now alarm-driven (wake uniformly 05:40–06:00, bed time derived from wake minus time in bed), weekends stay bed-time-driven so social jetlag still shows; bed/wake clock times now bracket a modelled time-in-bed (`sleep_hours / efficiency`, efficiency ~0.90 → 0.93 after intervention 1) instead of the old wake jitter that let logged sleep exceed time in bed; regenerated demo DB + CSV.
- 2026-07-20: Fixed `estimate_e1rm` NaN guard, single-date `st.slider` crashes, sleep-tab NaN filter, and SQL-identifier validation in the Liftosaur body-measurements sync; retired the historical-weight import script; added `lib/ui.py` shared helpers (date-range slider, range filter, default Plotly layout) and a semantic `PALETTE` in `lib/constants.py`, adopted across tabs; UI consistency sweep (labels, sentence-case headers, empty states, rebuilt PR table); lib correctness pass (`RTS_TABLE` trimmed to singles, `Bench Press` key standardized, `ttl=3600` on cached loaders, tab-level caching, `compute_totals` extracted to `lib/calculations.py`); scripts polish (cronometer gitignore spelling, demo `END_DATE` derived from `date.today()`, backup-script git guard, narrowed excepts); tooling added (ruff + `pyproject.toml`, GitHub Actions CI running ruff + pytest, curated `requirements.txt`, devcontainer cleanup, `cronometer-export` binary removed from the working tree); README written, stale `body_measurement_calculators.xlsx` references reworded as provenance; test suite grown to 49.
