...
def render(checkin_df: pd.DataFrame | None) -> None:
    ...
    if "intervention" in checkin_df.columns:
        ...
        ivn_df = (
            checkin_df.loc[mask, ["date", "intervention"]]
            .rename(columns={"intervention": "label"})
    ...
    fig_sched.add_hline(
        y=tgt_wake_h,
        line_dash="dash",
        line_color="rgba(232,200,76,0.55)",
        line_width=1.5,
    ...
    fig_sched.add_hline(
        y=tgt_bed_h,
        line_dash="dash",
        line_color="rgba(76,155,232,0.55)",
        line_width=1.5,
    ...
    if not ivn_df.empty:
        ...
        nights_a = (
            nights_a_pool.tail(len(nights_b))
            if len(nights_a_pool) >= len(nights_b)
            else nights_a_pool
        )
        default_a = (
    ...
    if n_a == 0 or n_b == 0:
        st.info(
            "One of the selected periods has no nights with bed/wake time logged yet. "
    ...
    st.caption(
        ...
        "'n' = number of nights with bed/wake times logged in that period."
    )
