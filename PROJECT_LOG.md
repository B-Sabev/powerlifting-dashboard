...
- **Public repo/app uses synthetic demo data** — `data/powerlifting.db` and `data/daily_checkin.csv`
  are now gitignored (real files, stay local). `lib/constants.py` falls back to `data/demo_*.db/csv`
  when the real files are absent (Streamlit Cloud). Demo data is generated by
  `scripts/generate_demo_data.py` (idempotent, SEED=42). History rewritten with `git filter-repo`
  on 2026-06-29 to purge real files from past commits.
- **Backup: private GitHub repo + scoped SSH deploy key** — `data/powerlifting.db` and
  `data/daily_checkin.csv` are pushed daily to a private repo (`powerlifting-data-backup`) via
  `scripts/backup_data.sh`, the last cron step. Private because it holds the personal data just
  purged from the public repo. Deploy key scoped to that repo only (not an account-wide PAT).
