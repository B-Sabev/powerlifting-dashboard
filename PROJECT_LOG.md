...
- **Public repo/app uses synthetic demo data** — `data/powerlifting.db` and `data/daily_checkin.csv`
  ...
  `scripts/generate_demo_data.py` (idempotent, SEED=42). Git history rewrite with `git filter-repo`
  still needed to purge the real files from past commits — see plan file for exact commands.
