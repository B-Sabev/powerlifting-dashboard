...
from lib.calculations import compute_totals, estimate_e1rm
...
def build_totals_df(sets_df: pd.DataFrame, weight_df: pd.DataFrame) -> pd.DataFrame:
    """Cached wrapper around lib.calculations.compute_totals — see there for the
    (pure, unit-tested) totals/DOTS logic."""
    return compute_totals(sets_df, weight_df)
